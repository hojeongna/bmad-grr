...
> **Architectural decisions deserve multiple review lenses.** Load these skills before making each decision category. Apply them in combination — each lens catches what the others miss.

**IF `{eng_review_skill}` exists (REQUIRED):** Load the FULL file via Read tool. Apply its framework to **every decision**:

- **Scope Challenge**: Is this decision necessary, or is a simpler alternative available?
- **Failure Modes**: For this decision, what goes wrong in production? Be specific (race conditions, cache staleness, integration timeouts, nil propagation).
- **Test Coverage Gap**: What tests does this decision require? Are those tests feasible with the chosen stack?
- **Prior Learnings** (if `{learn_skill}` also installed): Does past project knowledge inform this decision?

**IF `{cso_skill}` exists AND the decision touches security** (auth, data handling, secrets, LLM trust boundaries, network exposure, user input): 

Load the FULL `{cso_skill}` file via Read tool. Apply its Chief Security Officer framework:

- **OWASP Top 10**: Does this decision expose any OWASP-listed risks?
- **STRIDE**: Spoofing / Tampering / Repudiation / Info Disclosure / DoS / Elevation of privilege threats?
- **Supply Chain**: Does this depend on a package/service that could be compromised?
- **Secrets Archaeology**: Is there a risk of accidentally committing credentials?
- **LLM Security**: Prompt injection, tool trust boundaries, data leakage concerns?

**IF `{investigate_skill}` exists AND the decision involves significant uncertainty** (novel tech, unusual constraint, conflicting requirements, conflicting team opinions):

Load the FULL `{investigate_skill}` file via Read tool. Apply its 4-phase systematic framework:

1. **Investigate**: What evidence do we have for each option? (benchmarks, case studies, community sentiment)
2. **Analyze**: Pattern match against known solutions and past failures
3. **Hypothesize**: Which approach best fits the constraints, and what would prove it wrong?
4. **Implement**: Document the decision with explicit reasoning and the disproving criteria

**IF `{learn_skill}` exists:** Query for past decisions on similar problems. If the team has a convention, surface it.

**IF any skills missing:** Warn clearly and continue with reduced rigor:

> ⚠️ `{skill_name}` not installed — this decision category will proceed without its framework. Install gstack to enable full multi-lens review.
