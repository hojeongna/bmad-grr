...
Before generating starter template analysis, load gstack skills to apply structured evaluation frameworks to each option.

**IF `{eng_review_skill}` exists (gstack installed):** Load the FULL file via Read tool. Internalize its Engineering Feasibility, Scope Challenge, and Prior Learnings frameworks.

Apply `plan-eng-review` lens to each starter option:

- **Scope Challenge**: Does the starter bring more than we need? Is there a leaner option that still delivers the core capability?
- **Failure Modes**: For each starter, what are realistic production failure modes? (dependency conflicts, update cadence, ecosystem health)
- **Prior Learnings Integration**: If `{learn_skill}` installed, query for past starter/framework choices — does the team have a strong preference from past projects?
- **Test Coverage**: Does the starter include testing patterns we can reuse, or will we have to bolt on a test framework?

**IF `{learn_skill}` exists:** Load the FULL file via Read tool and query for `architecture` and `preference` entries about past starter/framework choices. Surface relevant prior decisions — don't re-litigate what the team has already decided.

**IF `{health_skill}` exists AND this is a brownfield project:** Load and run `/health` on the existing codebase to establish a baseline. The starter should integrate with (not fight) the existing code health profile.

**IF skills missing:** Warn clearly and continue with standard starter evaluation:

> ⚠️ `{skill_name}` not installed — starter evaluation will proceed without structured framework. Install gstack to enable.
