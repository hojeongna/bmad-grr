...
> **This is the MOST important quality gate of the architecture workflow.** The complete architecture gets pressure-tested through 4 independent review lenses + systematic failure mode analysis + final security pass.
...
Apply the autoplan pipeline to the **complete architecture document** (loaded earlier in this step):

**🎯 CEO Review pass**
- Does the architecture serve the PRD's actual premise?
- Is there complexity that doesn't map to any business value?
- Could a leaner architecture deliver the same outcomes?
- Is there scope drift from what the PRD requires?

**🎨 Design Review pass** (if applicable — user-facing systems)
- Does the architecture support the UX vision from the PRD?
- Are frontend/backend boundaries aligned with user journeys?
- Is state management coherent from the user's perspective?

**🔧 Eng Review pass**
- Is every architectural decision feasible with the chosen stack?
- Are failure modes thought through for each component?
- Is testability built into the architecture?
- Are there performance / scale bottlenecks hidden in the design?
- Are dependencies explicit and versioned?

**🛠️ DX Review pass**
- Is the architecture developer-friendly?
- Are API boundaries clear and consistent?
- Is local development / debugging possible?
- Are error messages / observability planned?
- Can a new developer onboard to a single component without loading the whole system?

**IF `{systematic_debugging_skill}` exists (REQUIRED — superpowers):** Load the FULL file via Read tool. Apply its **Failure Mode Analysis** to each architectural component:

- For each component, systematically ask "how does this fail in production?"
- Identify cascading failure chains (component A fails → component B fails → ...)
- Flag components without explicit failure handling, retry logic, or graceful degradation
- Surface findings in the validation report

**IF `{cso_skill}` exists:** Apply a final security pass on the complete architecture — verify that previous step-04 security decisions are still valid in the holistic design. Look for:

- Unintended trust boundaries between components
- Secrets or credentials that flow across more boundaries than necessary
- Attack surfaces that grew from the sum of individual decisions

**Present findings per-review separately** — do NOT merge. Keep reviews clearly labeled (🎯 CEO / 🎨 Design / 🔧 Eng / 🛠️ DX / 💥 Failure Modes / 🔐 Security) so the user can address each perspective independently.

**IF `{autoplan_skill}` is missing but individual review skills are installed:** Run each plan-*-review individually (plan-ceo-review, plan-design-review, plan-eng-review, plan-devex-review).

**IF `{learn_skill}` exists:** After validation, save significant architectural decisions as `architecture`-type learnings so future projects can benefit.

**IF NONE of these skills exist:** Warn clearly and proceed with standard architecture validation:

> ⚠️ `gstack/autoplan` and individual review skills not installed — final 4-way validation will be skipped. Architecture will be validated with standard checks only. Install gstack for comprehensive multi-lens review.
