...
The user sees a clear summary of refinements, then chooses what to do next: chain immediately into dev-story for implementation, run design-pass Mode P first to check the refined story against an existing HTML mockup (only when one exists), or end the workflow with the refined story already queued for a future dev-story run.
...
- `[U]` Run design-pass Mode P first — load `{designPassCommand}` with the refined story and the mockup path so anything the mockup shows and the story still misses gets promoted into AC / Tasks. Offer this only when a `design-handoff` mockup exists for the UI in question. After design-pass completes, return here and re-offer `D` / `S` (the `U` option is consumed).
