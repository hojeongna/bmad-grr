...
### 2c. Comprehensive 4-Way Review via autoplan (REQUIRED — gstack)

> **This is the MOST important quality gate of the PRD workflow.** The complete PRD gets pressure-tested through 4 independent review lenses before final polish.

**IF `{autoplan_skill}` exists (gstack installed):** Load the FULL file via Read tool. Internalize its 4-way review pipeline philosophy — CEO, design, eng, DX — each reading the PRD from a distinct perspective.

Apply the autoplan pipeline to the COMPLETE PRD document (loaded in Section 1):

**🎯 CEO Review pass**
- Does the PRD have a clear premise? Is the problem the real problem?
- Is MVP scope right? (Still time to cut or focus)
- Is there a 10x version worth considering?
- Is there a narrower wedge we should take instead?
- Would this matter in 2 years?

**🎨 Design Review pass**
- Are user journeys coherent across sections?
- Are empty/error/loading states planned?
- Is the UX vision clear and consistent?
- Is accessibility mentioned where relevant?
- Does the UX match the emotional arc of the product?

**🔧 Eng Review pass**
- Are functional requirements implementable?
- Are non-functional requirements realistic (perf, scale, reliability)?
- Are there hidden architectural complexities?
- Is testability considered?
- Are there dependencies or integrations that aren't scoped?

**🛠️ DX Review pass** (if applicable — API/SDK/dev tool products)
- Is the developer experience planned?
- Are APIs well-specified?
- Is there a "golden path" for developers?
- Are error messages / debugging thought through?

**Present findings per-review separately** — do NOT merge the 4 reviews. Keep them clearly labeled (🎯 CEO / 🎨 Design / 🔧 Eng / 🛠️ DX) so the user can address each perspective independently.

Surface **all findings** that could improve the PRD. The user decides in Section 6 (menu) whether to incorporate them via Section 3 (Optimization Actions).

**IF `{autoplan_skill}` is missing but individual review skills are installed:**

Try to run each review individually:

- **IF `{ceo_review_skill}` exists**: Apply its full framework (Premise, Scope, Innovation, Temporal)
- **IF `{design_review_skill}` exists**: Apply its full framework
- **IF `{eng_review_skill}` exists**: Apply its full framework
- **IF `{devex_review_skill}` exists**: Apply its full framework

**IF NONE of these skills exist:** Warn clearly and proceed with standard document polish only:

> ⚠️ `gstack/autoplan` and individual review skills not installed — the 4-way validation will be skipped. PRD will proceed with standard polish only. Install gstack for comprehensive pre-finalization review.
