...
### 2c. Load Discovery Skills (REQUIRED — gstack)

Before starting the discovery conversation, load these REQUIRED skills to enrich the dialogue with structured frameworks:

**IF `{learn_skill}` exists (gstack installed):** Load the FULL file via Read tool. Query past project learnings using keywords from the detected project type/domain so far. Capture relevant entries (especially `architecture`, `pattern`, `preference` types) as `prior_learnings`. These inform discovery questions and prevent re-litigating previously-decided matters.

**IF `{office_hours_skill}` exists:** Load the FULL file via Read tool. Internalize its **6 forcing questions** framework:

1. **Demand reality** — is there actual demand or just assumed?
2. **Status quo** — what do users currently do, and why is that insufficient?
3. **Desperate specificity** — who *exactly* is desperate for this?
4. **Narrowest wedge** — what's the smallest valuable version?
5. **Observation** — what would you see if users loved this?
6. **Future-fit** — does this still matter in 2 years?

**Blend these questions naturally into Section 3 below** — don't dump all 6 at once. Weave them in as the conversation flows.

**IF `{ceo_review_skill}` exists:** Load the FULL file via Read tool. Internalize its Premise Challenge, Existing Code Leverage, and Dream State Mapping frameworks. Apply the **Premise Challenge** lens throughout discovery: *"Is the stated problem the real problem?"* Use **Dream State Mapping** when the user describes the product — push them toward 10x ambition (note as aspiration; do NOT force scope expansion without user approval).

**IF any of these skills are missing:** Warn clearly and continue with reduced depth:

> ⚠️ `{skill_name}` not installed — discovery will proceed without its framework. Install gstack to enable full depth.
