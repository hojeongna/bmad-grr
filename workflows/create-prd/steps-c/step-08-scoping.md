...
### 1b. Nuclear Scope Challenge (REQUIRED — plan-ceo-review)

> **This is where 80% of scope bloat gets caught.** Before defining MVP strategy, apply the CEO/founder lens to pressure-test the proposed scope.

**IF `{ceo_review_skill}` exists:** Load the FULL file via Read tool. Focus on its **Nuclear Scope Challenge** section (Step 0 of plan-ceo-review). Internalize these pressure tests:

- **Premise Challenge**: Is the MVP scope the real MVP, or a bloated "MVP+"? What's the *actual* minimum that validates the core hypothesis?
- **Existing Code Leverage**: Is there existing code/feature/open-source that already solves part of this? What would cost ~0 to reuse?
- **Dream State Mapping**: What would the 10x version look like? (Note as aspiration — do NOT expand scope without user approval)
- **Temporal Interrogation**: Does this still matter in 6 months? 2 years? Is any of this tied to a fad that will evaporate?

**Apply these questions to the current PRD content** (loaded in Section 1). When the user proposes MVP scope in Section 2, pressure-test each item:

- "Is this absolutely required for the first release, or is it 'nice to have' in disguise?"
- "Could we defer this to Phase 2 and still validate the core hypothesis?"
- "Is there an existing solution (library, feature, workaround) that removes the need to build this?"

**IF the skill is missing:** Warn clearly and continue with standard MVP facilitation:

> ⚠️ `gstack/plan-ceo-review` not installed — Nuclear Scope Challenge skipped. Install gstack to enable full scope pressure testing.

### 1c. Engineering Feasibility Sanity Check (CONDITIONAL — plan-eng-review)

**IF `{eng_review_skill}` exists AND this PRD involves significant technical risk** (novel tech, performance-critical, scalability concerns, integration complexity):

Load the FULL file via Read tool. Apply its **Engineering Feasibility** and **Scope Challenge** framework to the proposed MVP. Identify:

- Touchpoints the MVP scope implies
- Architectural complexity hidden in the requirements
- Test coverage implications
- Failure modes that could bite us

Cite specific PRD content when flagging feasibility concerns. If the scope requires infrastructure that doesn't exist yet, surface this in the MVP discussion below.

**IF technical risk is low** (pure UI/content project): Skip this check.
**IF skill is missing but technical risk is high:** Warn and continue with user's engineering judgment.
