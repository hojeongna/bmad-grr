...
Story breakdown benefits from parallel analysis and TDD-aware decomposition. Load these skills before generating stories:

**IF `{parallel_agents_skill}` exists (REQUIRED — superpowers):** Load the FULL file via Read tool. Internalize its parallel dispatch pattern.

When processing multiple epics (or multiple stories within a large epic), use parallel sub-agents:

- One agent per epic (for cross-epic dependency analysis)
- Or one agent per story (for per-story feasibility deep-dive)
- Each agent returns structured findings that the main thread aggregates

This makes the story breakdown dramatically faster when epic count is high.

**IF `{eng_review_skill}` exists (REQUIRED):** Load the FULL file via Read tool. For each story being generated, apply engineering lens:

- **Complexity Estimate**: S / M / L based on touchpoints, risk, test work
- **Feasibility Check**: Does the story fit within one dev agent's context?
- **Failure Modes**: What realistic production failures does this story introduce?
- **Test Coverage Gap**: What tests does this story require? Do they exist in the architecture?

**IF `{tdd_skill}` exists (REQUIRED — superpowers):** Load the FULL file via Read tool. Internalize its RED-GREEN-REFACTOR cycle and testing anti-patterns guide.

For each story, decompose it into testable units:

- **TDD feasibility**: Can this story be built TDD? (99% yes, but edge cases exist)
- **Test sequence**: What's the test-first order for this story?
- **Anti-patterns to avoid**: Any mocking risks, test-only methods, behavior-testing pitfalls specific to this story?
- **Document these decisions** in each story's Dev Notes / testing section

**IF `{cso_skill}` exists AND the current epic touches security:** Apply CSO framework to each story within that epic — ensure every auth/data/secret/boundary story has explicit security acceptance criteria.

**IF `{design_review_skill}` exists AND the current epic is UI-heavy:** Apply design-review framework to each UI story — ensure each story has UX considerations (empty states, loading, errors, a11y).

**IF any skills missing:** Warn clearly and continue with standard story generation:

> ⚠️ `{skill_name}` not installed — story breakdown will proceed without its framework. Install to enable.
