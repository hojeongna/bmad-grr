...
Before designing the epics list, load these skills to apply value and feasibility lenses to each proposed epic.

**IF `{learn_skill}` exists (gstack installed):** Load the FULL file via Read tool. Query past project learnings for epic sizing, dependency patterns, and past breakdown strategies. Capture relevant entries — especially any `architecture` or `pattern` types about how epics were structured before.

**IF `{ceo_review_skill}` exists:** Load the FULL file via Read tool. For each proposed epic, apply CEO lens:

- **Value Test**: Does this epic deliver meaningful user value on its own?
- **Premise Challenge**: Is this epic the real first priority, or scope creep disguised as "foundation"?
- **10x Check**: Is there a 10x more valuable framing of this epic?
- **Dependency Check**: Does this epic stand alone, or does it require future epics to be meaningful?

**IF `{eng_review_skill}` exists:** Load the FULL file via Read tool. For each proposed epic, apply engineering lens:

- **Scope Challenge**: Is this epic a single coherent unit of work, or a Frankenstein of unrelated concerns?
- **Feasibility**: Can this epic be completed with the chosen architecture?
- **Failure Modes**: What architectural decisions from step-04 of create-architecture affect this epic?
- **Test Coverage**: What's the testability profile of this epic?

**IF `{cso_skill}` exists AND any proposed epic touches security** (auth, data, secrets, network boundaries, user input handling):

Load the FULL `{cso_skill}` file via Read tool and apply CSO framework to affected epics. Flag security epics that need dedicated threat modeling.

**IF any skills missing:** Warn clearly and continue with standard epic design:

> ⚠️ `{skill_name}` not installed — epic design will proceed without its framework. Install gstack to enable.
